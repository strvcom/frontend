...
To reduce image size you can use many [Optimization Tools](#optimization-tools), but before you choose one you should know how to optimize your images.
...
But if you would love to have bigger control, you will probably also want to use [Font Face Observer](https://fontfaceobserver.com). It's an amazing, lightweight tool to detect if a font has already loaded on your website, it works with font services, and self-hosted fonts as well, the Promise based API is straightforward, and the browser support is very good. Here's a simple usage which resolves [FOUT](#fout-foit-foft)for My Family font.
...
Also, think of [security](../effective-and-consistent-workflow/dependency-checking#security).
